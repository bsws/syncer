...
class DownloadHotels extends \Knp\Command\Command 
{

    protected function configure()
    {
        $this->setName("download:hotels")
            ->setDescription("Download the hotels for a given provider and place the data into a file.")
            ->addArgument(
                "provider",
                InputArgument::REQUIRED,
                "Please provide the provider for which do you want to download the data")
            ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if(empty($providerData)) {
            ...
            $fs = new Filesystem();
            try {
                $fileName = $app['settings']['hotelsDownloadDir'].$providerIdent."/hotels.json";
                $fs->touch($fileName);

...
class DownloadGeography extends \Knp\Command\Command 
{

    protected function configure()
    {
        $this->setName("download:geography")
            ->setDescription("Download the geography for a given provider and place the data into a file.")
            ->addArgument(
                "provider",
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if(empty($providerData)) {
            ...
            $geography = $client->GetGeography();

            //write the file down
            $fs = new Filesystem();
            try {
                $fileName = $app['settings']['geographyDownloadDir'].$providerIdent."/chrGeography.json";
                $fs->touch($fileName);
